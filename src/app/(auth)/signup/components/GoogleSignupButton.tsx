...
export function GoogleSignupButton({ className }: GoogleSignupButtonProps) {
  ...
  const handleClick = async () => {
    try {
      ...
      const authStartUrl = buildOAuthStartUrl("/google", GOOGLE_SIGNUP_CALLBACK_PATH, challenge);
  ...
  return (
    <div className={className}>
      <button
        ...
        onClick={handleClick}
        disabled={loading}
        className="flex w-full items-center justify-center gap-2.5 rounded-xl border border-border bg-background px-4 py-2.5 text-sm font-medium text-foreground transition-colors hover:bg-muted/60 disabled:cursor-not-allowed disabled:opacity-60"
      >
        {loading ? (
